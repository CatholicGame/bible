...
// Normalize newlines in string for regex logic if needed, but regex with [\s\S]*? handles both \n and \r\n perfectly
...
// 2. Replace Score and Check Button with Profile in Header
...
// Safely replace the block using regex
code = code.replace(
  /\{\/\* Score \*\/\}[\s\S]*?<Check size=\{13\} \/> Kiểm tra\r?\n\s*<\/motion\.button>/,
  profileStr
);
...
// 4. Add the hintEffects renderer right before the final closing div!
